...
class Rubycoin {
  ...
  private function decodeBase58($base58) {
    ...
    for ($i = 0; $i < strlen($base58); $i++) {
      $current = (string) strpos(Rubycoin::$base58chars, $base58[$i]);
  ...
  /**
   * Convert a 160-bit Rubycoin hash to a Rubycoin address
   *
   * @author theymos
   * @param string $hash160
   * @param string $addressversion
   * @return string Rubycoin address
   * @access public
   */
  public static function hash160ToAddress($hash160, $addressversion = RUBYCOIN_ADDRESS_VERSION) {
  ...
  /**
   * Convert a Rubycoin address to a 160-bit Rubycoin hash
   *
   * @author theymos
   * @param string $addr
   * @return string Rubycoin hash
   * @access public
   */
  ...
  /**
   * Determine if a string is a valid Rubycoin address
   *
   * @author theymos
   ...
   * @access public
   */
  public static function checkAddress($addr, $addressversion = RUBYCOIN_ADDRESS_VERSION) {
    $addr = self::decodeBase58($addr);
    if (strlen($addr) != 50) {
      return false;
    }
  ...
  /**
   * Convert the input to its 160-bit Rubycoin hash
   *
   * @param string $data
   * @return string
   * @access private
  ...
  /**
   * Convert a Rubycoin public key to a 160-bit Rubycoin hash
   *
   * @param string $pubkey
   * @return string
   * @access public
...
class RubycoinClientException extends ErrorException {
...
class RubycoinClient extends jsonrpc_client {

  /**
   * Create a jsonrpc_client object to talk to the Rubycoin server and return it,
   * or false on failure.
   *
   * @param string $scheme
   * "http" or "https"
   * @param string $username
   * User name to use in connection the Rubycoin server's JSON-RPC interface
  ...
  public function __construct($scheme, $username, $password, $address = "localhost", $port = 4938, $certificate_path = '', $debug_level = 0) {
    ...
    if (empty($username))
      throw new RubycoinClientException("Username must be non-blank");
    if (empty($password))
      throw new RubycoinClientException("Password must be non-blank");
    $port = (string) $port;
    if (!$port || empty($port) || !is_numeric($port) || $port < 1 || $port > 65535 || floatval($port) != intval($port))
      throw new RubycoinClientException("Port must be an integer and between 1 and 65535");
    if (!empty($certificate_path) && !is_readable($certificate_path))
      throw new RubycoinClientException("Certificate file " . $certificate_path . " is not readable");
  ...
  /**
   * Test if the connection to the Rubycoin JSON-RPC server is working
   *
  ...
  public function can_connect() {
    try {
      $r = $this->getinfo();
    } catch (RubycoinClientException $e) {
      return $e->getMessage();
    }
    return true;
  }
  ...
  /**
   * Convert a Rubycoin server query argument to a jsonrpcval
   *
  ...
  public function query($message) {
    if (!$message || empty($message))
      throw new RubycoinClientException("Rubycoin client query requires a message");
    ...
    if ($response->faultCode()) {
      throw new RubycoinClientException($response->faultString());
    }
  ...
  public function backupwallet($destination) {
    if (!$destination || empty($destination))
      throw new RubycoinClientException("backupwallet requires a destination");
   ...
   * @return float Rubycoin balance
   * @throws RubycoinClientException
   */
  public function getbalance($account = NULL, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('getbalance requires a numeric minconf >= 0');
    if ($account === NULL)
  ...
  /**
   * Returns boolean true if server is trying to generate Rubycoins, false otherwise.
   *
  ...
  /**
   * Tell Rubycoin server to generate Rubycoins or not, and how many processors
   * to use.
   *
  ...
  public function setgenerate($generate = TRUE, $maxproc = -1) {
    if (!is_numeric($maxproc) || $maxproc < -1)
      throw new RubycoinClientException('setgenerate: $maxproc must be numeric and >= -1');
  ...
  public function getaccount($address) {
    if (!$address || empty($address))
      throw new RubycoinClientException("getaccount requires an address");
  ...
  public function getlabel($address) {
    if (!$address || empty($address))
      throw new RubycoinClientException("getlabel requires an address");
  ...
  public function setaccount($address, $account = "") {
    if (!$address || empty($address))
      throw new RubycoinClientException("setaccount requires an address");
  ...
  public function setlabel($address, $label = "") {
    if (!$address || empty($address))
      throw new RubycoinClientException("setlabel requires an address");
   ...
   * @return float Rubycoin total
   * @throws RubycoinClientException
   */
  public function getreceivedbyaddress($address, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('getreceivedbyaddress requires a numeric minconf >= 0');
    if (!$address || empty($address))
      throw new RubycoinClientException("getreceivedbyaddress requires an address");
  ...
  public function getreceivedbyaccount($account, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('getreceivedbyaccount requires a numeric minconf >= 0');
    if (!$account || empty($account))
      throw new RubycoinClientException("getreceivedbyaccount requires an account");
  ...
  public function getreceivedbylabel($label, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('getreceivedbylabel requires a numeric minconf >= 0');
    if (!$label || empty($label))
      throw new RubycoinClientException("getreceivedbylabel requires a label");
  ...
  /**
   * Return an array of arrays showing how many Rubycoins have been received by
  ...
  public function listreceivedbyaddress($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('listreceivedbyaddress requires a numeric minconf >= 0');
  ...
  public function listreceivedbyaccount($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('listreceivedbyaccount requires a numeric minconf >= 0');
  ...
  public function listreceivedbylabel($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('listreceivedbylabel requires a numeric minconf >= 0');
   ...
   *
   * @param string $address Destination Rubycoin address or IP address
  ...
  public function sendtoaddress($address, $amount, $comment = NULL, $comment_to = NULL) {
    if (!$address || empty($address))
      throw new RubycoinClientException("sendtoaddress requires a destination address");
    if (!$amount || empty($amount))
      throw new RubycoinClientException("sendtoaddress requires an amount to send");
    if (!is_numeric($amount) || $amount <= 0)
      throw new RubycoinClientException("sendtoaddress requires the amount sent to be a number > 0");
  ...
  /**
   * Check that $address looks like a proper Rubycoin address.
   *
   * @param string $address String to test for validity as a Rubycoin address
   * @return array An array containing:
   * "isvalid" => true or false
   * "ismine" => true if the address is in the server's wallet
   * "address" => Rubycoinaddress
   * Note: ismine and address are only returned if the address is valid.
   * @throws RubycoinClientException
   */
  public function validateaddress($address) {
    if (!$address || empty($address))
      throw new RubycoinClientException("validateaddress requires a Rubycoin address");
  ...
  public function gettransaction($txid) {
    if (!$txid || empty($txid) || strlen($txid) != 64 || !preg_match('/^[0-9a-fA-F]+$/', $txid))
      throw new RubycoinClientException("gettransaction requires a valid hexadecimal transaction ID");
    return $this->query("gettransaction", $txid);
  }

  /**
   * Move Rubycoins between accounts.
   *
   * @param string $fromaccount
   * Account to move from. If given as an empty string ("") or NULL, Rubycoins will
   ...
   * Amount to move
   * @param integer $minconf
   * Minimum number of confirmations on Rubycoins being moved
   * @param string $comment
   * Transaction comment
   * @throws RubycoinClientException
   * @since 0.3.18
  ...
  public function move($fromaccount = "", $toaccount, $amount, $minconf = 1, $comment = NULL) {
    ...
    if (!$amount || !is_numeric($amount) || $amount <= 0)
      throw new RubycoinClientException("move requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('move requires a numeric $minconf >= 0');
   ...
   * if there is less than $amount Rubycoins with $minconf confirmations in the
   ...
   * @param string $toaddress Rubycoin address to send to
   * @param float $amount Amount to send
   * @param integer $minconf Minimum number of confirmations on Rubycoins being sent
   * @param string $comment
   * @param string $comment_to
  ...
  public function sendfrom($account, $toaddress, $amount, $minconf = 1, $comment = NULL, $comment_to = NULL) {
    if (!$account || !$toaddress || empty($toaddress) || !$amount || !is_numeric($amount) || $amount <= 0)
      throw new RubycoinClientException("sendfrom requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('sendfrom requires a numeric $minconf >= 0');
    if (!$comment && !$comment_to)
  ...
  /**
   * Return the current Rubycoin address for receiving payments to $account.
   * The account and address will be created if $account doesn't exist.
   *
   * @param string $account Account name
   * @return string Rubycoin address for $account
   * @throws RubycoinClientException
   * @since 0.3.18
   */
  public function getaccountaddress($account) {
    if (!$account || empty($account))
      throw new RubycoinClientException("getaccountaddress requires an account");
   ...
   * @param string $account
   * @return array
   * A simple array of Rubycoin addresses associated with $account, empty
   * if the account doesn't exist.
   * @throws RubycoinClientException
   */
  public function getaddressesbyaccount($account) {
    if (!$account || empty($account))
      throw new RubycoinClientException("getaddressesbyaccount requires an account");
...
if (!$account) $account = "";
    ...
    if (!is_numeric($count) || $count < 0)
      throw new RubycoinClientException('listtransactions requires a numeric count >= 0');
    if (!is_numeric($from) || $from < 0)
      throw new RubycoinClientException('listtransactions requires a numeric from >= 0');
  ...
  public function sendmany($fromAccount, $sendTo, $minconf = 1, $comment=NULL) {
    if (!$fromAccount || empty($fromAccount))
      throw new RubycoinClientException("sendmany requires an account");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new RubycoinClientException('sendmany requires a numeric minconf >= 0');
